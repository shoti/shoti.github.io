...
function validateBriefing(briefing) {
  ...
  briefing.stories.forEach((story, storyIndex) => {
    ...
    requireString(story.category, `${location}.category`, { min: 2, max: 50 });
    if (!CATEGORY_PATTERN.test(story.category)) {
      fail(`${location}.category`, 'must be a lowercase hyphenated slug');
    }
